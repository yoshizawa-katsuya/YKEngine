...
void DirectXCommon::CreateRenderTexture()
{
	...
	renderTextureRtvHandle_ = GetRTVCPUDescriptorHandle(2);
...
void DirectXCommon::PostDrawRenderTexture(PrimitiveDrawer* primitiveDrawer, SrvHeapManager* srvHeapManager)
{
	...
	//コピー実行
	primitiveDrawer->SetPipelineSet(commandList_.Get(), BlendMode::kOffScreenRendering);
...
Microsoft::WRL::ComPtr<ID3D12Resource> DirectXCommon::CreateRenderTextureResource(int32_t width, int32_t height, DXGI_FORMAT format, const Vector4& clearColor)
{
	//生成するResourceの設定
	D3D12_RESOURCE_DESC resourceDesc{};
	resourceDesc.Width = width;	//Textureの幅
...
void DirectXCommon::CreateRenderTextureSRV(SrvHeapManager* srvHeapManager)
{
